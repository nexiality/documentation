...
{var preresult='';var postresult='';var out='';var show_replace=false;var show_stop=false;var standard=true;var c=0;found=[];var d_o=$('#tipue_search_input').val();var d=d_o.toLowerCase();d=$.trim(d);if((d.match("^\"")&&d.match("\"$"))||(d.match("^'")&&d.match("'$")))
{standard=false;}
...
{if(set.wholeWords)
{var pat=new RegExp('\\b'+escapeForRegex(d_w[f])+'\\b','gi');}
else
{var pat=new RegExp(escapeForRegex(d_w[f]),'gi');}
...
{if(tipuesearch_in.pages[i].url.startsWith(tipuesearch_weight.weight[e].url))
{score+=tipuesearch_weight.weight[e].score;}}}
if(d_w[f].match('^-'))
{pat=new RegExp(escapeForRegex(d_w[f].substring(1)),'i');if(tipuesearch_in.pages[i].title.search(pat)!=-1||tipuesearch_in.pages[i].text.search(pat)!=-1||tipuesearch_in.pages[i].tags.search(pat)!=-1)
{score=0;}}}
if(score!=0)
{found.push({"score":score,"title":tipuesearch_in.pages[i].title,"desc":s_t,"url":tipuesearch_in.pages[i].url});c++;}}}
else
{for(var i=0;i<tipuesearch_in.pages.length;i++)
{var score=0;var s_t=tipuesearch_in.pages[i].text;var pat=new RegExp(escapeForRegex(d),'gi');if(tipuesearch_in.pages[i].title.search(pat)!=-1)
{var m_c=tipuesearch_in.pages[i].title.match(pat).length;score+=(20*m_c);}
if(tipuesearch_in.pages[i].text.search(pat)!=-1)
{var m_c=tipuesearch_in.pages[i].text.match(pat).length;score+=(20*m_c);}
if(tipuesearch_in.pages[i].tags.search(pat)!=-1)
...
if(c==1)
{preresult+='<div id="tipue_search_results_count">'+tipuesearch_string_4;}
else
{c_c=c.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",");preresult+='<div id="tipue_search_results_count">'+c_c+' '+tipuesearch_string_5;}
if(set.showTime)
{var endTimer=new Date().getTime();var time=(endTimer-startTimer)/ 1000;preresult+=' ('+time.toFixed(2)+' '+tipuesearch_string_14+')';set.showTime=false;}
preresult+='</div>';found.sort(function(a,b){return b.score-a.score});var l_o=0;for(var i=0;i<found.length;i++)
...
{var patr=new RegExp('('+escapeForRegex(d_w[f])+')','gi');t=t.replace(patr,"<h0011>$1<h0012>");}}}
else if(set.highlightTerms)
{var patr=new RegExp('('+escapeForRegex(d)+')','gi');t=t.replace(patr,"<span class=\"tipue_search_content_bold\">$1</span>");}
...
{out+='</div>';}}
if(c>set.show)
{var pages=Math.ceil(c / set.show);var page=(start / set.show);postresult+='<nav><div id="tipue_search_foot"><ul id="tipue_search_foot_boxes">';if(start>0)
{postresult+='<li role="navigation"><a class="tipue_search_foot_box" accesskey="b" id="'+(start-set.show)+'_'+replace+'">'+tipuesearch_string_6+'</a></li>';}
if(page<=2)
{var p_b=pages;if(pages>3)
{p_b=3;}
for(var f=0;f<p_b;f++)
{if(f==page)
{postresult+='<li class="current" role="navigation">'+(f+1)+'</li>';}
else
{postresult+='<li role="navigation"><a class="tipue_search_foot_box" id="'+(f*set.show)+'_'+replace+'">'+(f+1)+'</a></li>';}}}
else
...
{postresult+='<li role="navigation"><a class="tipue_search_foot_box" id="'+(f*set.show)+'_'+replace+'">'+(f+1)+'</a></li>';}}}
if(page+1!=pages)
{postresult+='<li role="navigation"><a class="tipue_search_foot_box" accesskey="m" id="'+(start+set.show)+'_'+replace+'">'+tipuesearch_string_7+'</a></li>';}
postresult+='</ul></div></nav>';}}
else
...
$('#tipue_search_content').hide().html(tipuesearch_string_16+preresult+'<div id="search_preview">'+out+'</div>'+postresult).slideDown(200);$('#tipue_search_replaced').click(function()
...
function escapeForRegex(text) { return text.replace(/([\[\]{}()#$%^&*\-+=><|\\\/?.,])/g, '\\$1'); }