...
2. #### `artifact`
   ...
   1. #### `artifact/script`
      ...
      Each sheet (test scenario) two sections: header (description of a test scenario) and body (test steps to 
      automate). For better understanding about scenario and test steps of test scripts, script developer can provide 
      information in `description` section of respective sections, as such test object and user acceptance criteria.

      It is recommended that the [macro files](../commands/base/macro(file,sheet,name)) are stored in this location as 
      well.
